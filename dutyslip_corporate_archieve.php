...
$printid=$_GET['printid'];
  $sql = mysqli_query($conn,"SELECT * FROM `duty_slip_corporate` WHERE `id`='$printid'");
...
www.takeourtrip.com, reservation@takeourtrip.com </h4>
    ...
    <td colspan="2" height="20" --="text-align: center;">DUTY SLIP</td>
  ...
  <tr>
    <td colspan="9" height="10">
      <p style="float: left; padding-left: 10px;">Corporate's Name:<?php echo($rowdata['corporate_name']); ?></p>
      <p style="float: left; padding-left: 10px;">Driver's Name:<?php echo($rowdata['driver_name']); ?></p><p style="float: left; padding-left: 500px">Duty Of :<?php echo($rowdata['duty_of']); ?></p><p style="float: right; padding-right: 200px;">Car No.:<?php echo($rowdata['vehicle_no']); ?></p></td>
    
  </tr>
  <?php
     $sql = mysqli_query($conn,"SELECT * FROM `duty_slip` WHERE `id`='$printid'");
  while($rowdata = mysqli_fetch_assoc($sql)){
    ?>
  <tr>
...
<?php } ?>
<?php $sql = mysqli_query($conn,"SELECT * FROM `duty_slip_corporate` WHERE `id`='$printid'");
  while($rowdata = mysqli_fetch_assoc($sql)){ ?>
